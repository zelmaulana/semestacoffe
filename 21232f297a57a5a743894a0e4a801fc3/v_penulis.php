<?php
if(isset($_POST['add'])){
	mysqli_query($koneksi, "INSERT INTO l_penulis VALUES ('','$_POST[nama]','$_POST[alamat]','$_POST[email]','$_POST[nohp]')");
	?>
    <script>
...
if(isset($_POST['updt'])){
	mysqli_query($koneksi, "UPDATE l_penulis SET penulis_name='$_POST[nama]', alamat='$_POST[alamat]', email='$_POST[email]', no_hp='$_POST[nohp]' WHERE penulis_id = '$_GET[id]'");
	?>
    <script>
	alert("Data Berhasil Disimpan");
	window.location="?i=<?php echo $_GET['i']?>";
...
<?php
	$no = 1;
	$sql = mysqli_query($koneksi, "SELECT * FROM l_penulis");
	while ($hasil = mysqli_fetch_assoc($sql)){
?> 
     
                <tr>
                  <td><?php echo $no ?></td>
                  <td><?php echo $hasil['penulis_name']?></td>
                  <td><?php echo $hasil['alamat']?></td>
                  <td><?php echo $hasil['email']?></td>
                  <td><?php echo $hasil['no_hp']?></td>
                  <td><a href="?i=<?php echo $_GET['i']?>&page=updt&id=<?php echo $hasil['penulis_id']?>">
                  <button type="button" class="btn btn-success fa fa-edit"> Edit</button></a>
                  <a href="?i=<?php echo md5('d_penulis')?>&id=<?php echo $hasil['penulis_id']?>">
                ...
                <tfoot>
					...
					if(isset($_GET['page'])){
						if($_GET['page'] == 'add'){
							...
							$sp = mysqli_query($koneksi, "SELECT * FROM l_penulis WHERE penulis_id = '$_GET[id]'");
							$hp = mysqli_fetch_array($sp);
							?>
							<tr>
                                <td></td>
                                <td><input type="text" class="form-control" style="width:100%" name="nama" value="<?php echo $hp['penulis_name']?>" required /></td>
