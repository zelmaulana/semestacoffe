	...
	$tgl = date('Y-m-d G:i:s');
			mysqli_query($koneksi, "INSERT INTO t_pemesanan VALUES ('','$_SESSION[id]', '$ha[biaya]', '$tot', '1', '$tgl', '', '1')");
			$maxq = mysqli_query($koneksi, "SELECT MAX(pemesanan_id) as aa FROM t_keranjang
								    		WHERE user_id = '$_SESSION[id]'");
			$max = mysqli_fetch_array($maxq);
			?>
            <script>
				alert('Terimakasih Pesanan Anda Kami Terima, Untuk Selanjutnya, Silahkan Melakukan Pembayaran');
				window.location="?i=bayar&idp=<?php echo $max['aa'] ?>";
			</script>
            <?php
		}
		else if($_GET['id'] == $b){
	...
	$time = date("Y-m-d");
	$tgl = date('Y-m-d G:i:s');
			mysqli_query($koneksi, "INSERT INTO t_pemesanan VALUES ('','$_SESSION[id]', '$ha[biaya]', '$tot', '1', '$tgl', '', '2')");
			$maxq = mysqli_query($koneksi, "SELECT MAX(pemesanan_id) as aa FROM t_keranjang
								    		WHERE user_id = '$_SESSION[id]'");
			$max = mysqli_fetch_array($maxq);
			?>
